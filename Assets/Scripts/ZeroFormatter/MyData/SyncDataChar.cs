...
namespace MyData
{
    ...
    public class SyncDataCharFormatter<TTypeResolver> : Formatter<TTypeResolver, global::MyData.SyncDataChar>
        where TTypeResolver : ITypeResolver, new()
    {
        ...
        public override int Serialize(ref byte[] bytes, int offset, global::MyData.SyncDataChar value)
        {
            ...
            else
            {
                ...
                offset += ObjectSegmentHelper.SerializeFromFormatter<TTypeResolver, char>(ref bytes, startOffset, offset, 2, value.Value);

                return ObjectSegmentHelper.WriteSize(ref bytes, startOffset, offset, 2);
            }
        }

        public override global::MyData.SyncDataChar Deserialize(ref byte[] bytes, int offset, global::ZeroFormatter.DirtyTracker tracker, out int byteSize)
        {
            ...
            return new SyncDataCharObjectSegment<TTypeResolver>(tracker, new ArraySegment<byte>(bytes, offset, byteSize));
        }
    }

    public class SyncDataCharObjectSegment<TTypeResolver> : global::MyData.SyncDataChar, IZeroFormatterSegment
        where TTypeResolver : ITypeResolver, new()
    {
        static readonly int[] __elementSizes = new int[]{ 16, 0, 2 };
        ...
        public override char Value
        {
            get
            {
                return ObjectSegmentHelper.GetFixedProperty<TTypeResolver, char>(__originalBytes, 2, __binaryLastIndex, __extraFixedBytes, __tracker);
            }
            set
            {
                ObjectSegmentHelper.SetFixedProperty<TTypeResolver, char>(__originalBytes, 2, __binaryLastIndex, __extraFixedBytes, value, __tracker);
            }
        }


        public SyncDataCharObjectSegment(global::ZeroFormatter.DirtyTracker dirtyTracker, ArraySegment<byte> originalBytes)
        {
        ...
        public int Serialize(ref byte[] targetBytes, int offset)
        {
            if (__extraFixedBytes != null || __tracker.IsDirty)
            {
                ...
                offset += ObjectSegmentHelper.SerializeFixedLength<TTypeResolver, char>(ref targetBytes, startOffset, offset, 2, __binaryLastIndex, __originalBytes, __extraFixedBytes, __tracker);
